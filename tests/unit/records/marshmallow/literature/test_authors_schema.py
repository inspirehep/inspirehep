...
from inspirehep.records.marshmallow.literature import LiteratureAuthorsSchemaV1


def test_authors_schema():
    schema = LiteratureAuthorsSchemaV1()
    authors = [
...
def test_authors_schema_without_authors():
    ...
    expected = {"metadata": {"collaborations": [{"value": "LHCb"}]}}
    result = json.loads(schema.dumps(data).data)
    assert expected == result
...
def test_authors_schema_without_authors_and_collaborations():
    ...
    data = {"metadata": data}
    expected = {"metadata": {"authors": [], "collaborations": []}}
    result = json.loads(schema.dumps(data).data)
    assert expected == result
