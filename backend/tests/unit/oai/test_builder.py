...
def collect_creators(record):
    return list(OpenAIREXMLBuilder()._iter_creators(record))


def collect_alts(record):
    return list(OpenAIREXMLBuilder()._iter_alternate_identifiers(record))
...
def test_rights_from_license(licenses, expected):
    builder = OpenAIREXMLBuilder()
    assert builder._rights_from_license(licenses) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"subtitle": "Just a Subtitle"}], ("", "Just a Subtitle")),
...
def test_compose_title(titles, expected):
    builder = OpenAIREXMLBuilder()
    assert builder._compose_title(titles) == expected
...
"""
    with patch(
        "inspirehep.oai.builder.get_inspirehep_url",
