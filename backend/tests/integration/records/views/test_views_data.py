...
def test_data_application_json_get(api_client, db, es, create_record_factory):
...
def test_data_application_json_put(api_client, db, es, create_record_factory):
    record = create_record_factory("dat", with_indexing=True)
    record_control_number = record.json["control_number"]
...
def test_data_application_json_delete(api_client, db, es, create_record_factory):
    record = create_record_factory("dat", with_indexing=True)
    record_control_number = record.json["control_number"]
...
def test_data_search_json_get(api_client, db, es, create_record_factory):
