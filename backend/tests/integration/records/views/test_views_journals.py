...
def test_journals_application_json_get(api_client):
...
def test_journals_application_json_put(api_client):
    record = create_record_factory("jou", with_indexing=True)
    record_control_number = record.json["control_number"]
...
def test_journals_application_json_delete(api_client):
    record = create_record_factory("jou", with_indexing=True)
    record_control_number = record.json["control_number"]
...
def test_journals_application_json_post(api_client):
    expected_status_code = 401
...
def test_journals_search_json_get(api_client):
...
def test_journal_record_search_results(api_client):
