...
def test_experiments_application_json_get(app_clean):
...
def test_experiments_application_json_put(app_clean):
    record = create_record_factory("exp", with_indexing=True)
    record_control_number = record.json["control_number"]
...
def test_experiments_application_json_delete(app_clean):
    record = create_record_factory("exp", with_indexing=True)
    record_control_number = record.json["control_number"]
...
def test_experiments_application_json_post(app_clean):
    expected_status_code = 401
    with app_clean.app.test_client() as client:
...
def test_experiments_search_json_get(app_clean):
...
def test_experiments_record_search_results(app_clean):
