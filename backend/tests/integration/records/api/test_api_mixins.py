...
def test_records_links_correctly_with_conference(app_clean):
...
def test_creating_lit_record_with_institution_linked_more_than_once(app_clean):
...
def test_updating_record_updates_institution_relations(app_clean):
...
def test_deleted_record_deletes_relations_in_institution_literature_table(app_clean):
    institution = create_record("ins")
    institution_control_number = institution["control_number"]
    ref = f"http://localhost:8000/api/institutions/{institution_control_number}"
...
def test_institution_literature_table_is_not_updated_when_feature_flag_is_disabled(
    update_function_mock, app_clean
):
    institution = create_record("ins")
    institution_control_number = institution["control_number"]
    ref = f"http://localhost:8000/api/institutions/{institution_control_number}"
...
def test_record_updates_correctly_conference_link(app_clean):
...
def test_record_links_only_existing_conference(app_clean):
    rec_data = {
        "publication_info": [
            {
                "conference_record": {
...
def test_conference_paper_doesnt_link_deleted_conference(app_clean):
    conference = create_record("con")
    conference_control_number = conference["control_number"]
    ref = f"http://localhost:8000/api/conferences/{conference_control_number}"
...
def test_delete_literature_clears_entries_in_conference_literature_table(app_clean):
    conference = create_record("con")
    conference_control_number = conference["control_number"]
    ref = f"http://localhost:8000/api/conferences/{conference_control_number}"
