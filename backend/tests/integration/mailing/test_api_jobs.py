...
def test_jobs_from_last_week_empty(app_clean):
...
@pytest.mark.vrc()
def test_subscirbe_to_the_list(app_clean, vcr_cassette):
    result = subscribe_to_jobs_weekly_list("luke@cage.com", "Luke", "Cage")
    assert vcr_cassette.all_played


def test_subscirbe_to_the_list_with_invalid_email(app_clean):
    with pytest.raises(ValueError):
        subscribe_to_jobs_weekly_list("luke", "Luke", "Cage")


def test_get_job_recipient_no_internal_uid(app_clean):
...
def test_get_job_recipient_internal_uid(app_clean):
    expected_email = "somebody@cern.ch"
