...
def test_minter_update_conference_record_with_different_cnum_raises_error(async_app):
...
def test_minter_undelete_conference_record_registers_deleted_pid(async_app):
    data = {"opening_date": "1994-12-10"}
    rec = ConferencesRecord.create(faker.record("con", data=data))
    db.session.commit()
...
def test_minter_undelete_conference_record_without_cnum(async_app):
