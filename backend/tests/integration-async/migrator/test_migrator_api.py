...
def test_continuous_migration(async_app):
    raw_record_citer = (
        b"<record>"
    ...
    async_app.redis.rpush("legacy_records", zlib.compress(raw_record_citer))
    async_app.redis.rpush("legacy_records", zlib.compress(raw_record_cited))
    async_app.redis.rpush("legacy_records", b"END")

    assert async_app.redis.llen("legacy_records") == 3
    ...
    assert async_app.redis.llen("legacy_records") == 0


def test_continuous_migration_with_an_invalid_record(async_app):
    raw_record_citer = (
        b"<record>"
        b'  <controlfield tag="001">666</controlfield>'
        b'  <datafield tag="245" ind1=" " ind2=" ">'
    ...
    async_app.redis.rpush("legacy_records", zlib.compress(raw_record_citer))
    async_app.redis.rpush("legacy_records", zlib.compress(raw_record_invalid))
    async_app.redis.rpush("legacy_records", zlib.compress(raw_record_cited))
    async_app.redis.rpush("legacy_records", b"END")

    assert async_app.redis.llen("legacy_records") == 4
...
def test_continuous_migration_with_different_type_of_records(async_app):
    raw_record_citer = (
        b"<record>"
        b'  <controlfield tag="001">666</controlfield>'
        b'  <datafield tag="245" ind1=" " ind2=" ">'
    ...
    async_app.redis.rpush("legacy_records", zlib.compress(raw_record_citer))
    async_app.redis.rpush("legacy_records", zlib.compress(raw_author))
    async_app.redis.rpush("legacy_records", zlib.compress(raw_record_cited))
    async_app.redis.rpush("legacy_records", b"END")
...
def test_continuous_migration_with_invalid_control_number(async_app):
    raw_record_citer = (
        b"<record>"
        b'  <controlfield tag="001">666</controlfield>'
        b'  <datafield tag="245" ind1=" " ind2=" ">'
    ...
    assert async_app.redis.llen("legacy_records") == 2
