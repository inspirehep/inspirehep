...
def test_valid_json_with_args(caplog):
    ...
    assert (
        result.data["error"]
        == "Error for workflow 1234: {'detail': 'Service unavailable'}"
    )
    assert "Error for workflow 1234" in caplog.text
...
def test_json_decode_error_with_response_text(caplog):
    ...
    assert result.status_code == 503
    assert result.data["error"] == "Custom error: Service Unavailable"
...
def test_no_response(caplog):
    ...
    assert result.data["error"] == "Error no response: Unknown error"
    assert "Error no response: Unknown error" in caplog.text
...
def test_no_args_no_response_text(caplog):
    ...
    assert result.data["error"] == "Simple error: {'error': 'fail'}"
