...
export class JsonEditorWrapperComponent extends SubscriberComponent
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      (changes['recordId'] || changes['recordType']) &&
      this.recordId &&
      this.recordType
    ) {
  ...
  onRevisionRevert() {
    this.globalAppStateService.jsonBeingEdited$.next(this.revision);
      ...
      .catch(error => {
        ...
        if (error.status === 403) {
          this.toastrService.error(
            `Logged in user can not access to the record: ${recordType}/${recordId}`,
            'Forbidden'
          );
        } else {
