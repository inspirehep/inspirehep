...
export class MultiEditorComponent implements OnInit {
  ...
  onSave() {
    let uuids = Object.keys(this.recordSelectionStatus).filter(
      key => this.recordSelectionStatus[key] !== this.allSelected
    );
    this.apiService
      .save(this.previewedActions, uuids, this.allSelected)
      .subscribe(
        res => {
  ...
  selectAll() {
    ...
    Object.keys(this.recordSelectionStatus).forEach(item => {
      this.recordSelectionStatus[item] = true;
    });
    this.changeDetectorRef.markForCheck();
  }
  ...
  deselectAll() {
    ...
    Object.keys(this.recordSelectionStatus).forEach(item => {
      this.recordSelectionStatus[item] = false;
    });
    this.changeDetectorRef.markForCheck();
  }
  ...
  previewActions() {
    if (!this.hasAnyNonEmptyAction(this.userActions)) {
      ...
      this.apiService
        .previewActions(this.userActions, this.currentPage, this.pageSize)
        .subscribe(
          res => {
  ...
  private queryCollection(query: string, collection: string) {
    ...
    this.searchSubscription = this.apiService
      .searchRecords(query, this.currentPage, collection, this.pageSize)
      .subscribe(
        json => {
          this.previewMode = false;
          this.records = json.json_records;
  ...
  onCollectionChange(selectedCollection: string) {
    ...
    this.apiService.fetchCollectionSchema(this.selectedCollection).subscribe(
      res => {
        this.schema = res;
  ...
  private filterRecord(record: object): object {
    if (this.filterExpressions && this.filterExpressions.size > 0) {
      return this.jsonFilterService.filterObject(
        record,
  ...
  private filterRecords() {
    this.filteredRecords = this.records.map(item => this.filterRecord(item));
    this.changeDetectorRef.markForCheck();
  }

  private hasAnyNonEmptyAction(userActions: UserActions): boolean {
    let existingAction = false;
    return userActions.actions.some(action => Boolean(action.mainKey));
  }
  ...
  private validateActionsKeypaths(userActions: UserActions): boolean {
    let invalid = userActions.actions.some(
      action => !this.schemaKeysStoreService.findSubSchema(action.mainKey)
    );
    if (!invalid) {
      invalid = userActions.conditions.some(
        action => !this.schemaKeysStoreService.findSubSchema(action.key)
      );
    }
    return !invalid;
  }
