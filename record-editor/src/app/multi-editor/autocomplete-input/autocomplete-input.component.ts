    ...
    this.dataSource = Observable.create((observer: any) => {
      ...
      .mergeMap((state: ParsedAutocompleteInput) => {
        return Observable.of(
          this.schemaKeysStoreService
            .forPath(state.path)
            .filter(item => item.startsWith(state.query))
        );
      });
    this.valueChange$.debounceTime(500).subscribe(value => {
  ...
  selectUserInput(match: TypeaheadMatch) {
    if (this.currentPath !== '') {
      this.value = `${this.currentPath}${
        this.schemaKeysStoreService.separator
      }${match.value}`;
    } else {
