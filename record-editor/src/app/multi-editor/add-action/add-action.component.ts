...
export class AddActionComponent {
  ...
  saveRecord(record: object) {
    ...
    this.action.value = this.subSchema['alwaysShow']
  ...
  onValueChange(value: string) {
    ...
    this.subSchema = this.schemaKeysStoreService.findSubSchema(
      this.action.mainKey
    );
    this.action.value = {};
  }
  ...
  openEditor() {
    if (!this.action.mainKey) {
      ...
      this.toastr.error(
        'Choose a key using autocompletion values for the field to be added'
      );
    } else {
