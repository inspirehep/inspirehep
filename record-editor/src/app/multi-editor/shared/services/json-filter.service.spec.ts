...
describe('JsonFilterService', () => {
  ...
  beforeEach(() => {
    service = new JsonFilterService(
      new RecordCleanupService(),
      new SchemaKeysStoreService()
    );
  });
  ...
  it('filters a nested complex object given a path and returns the object keeping the object structure.', () => {
    ...
    let result = service.filterObject(
      record,
      Set(['authors.affiliations.value', 'authors.full_name'])
    );
