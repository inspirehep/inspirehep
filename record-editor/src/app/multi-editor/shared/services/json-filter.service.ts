...
export class JsonFilterService {
  ...
  private filterObjectRecursively(
    json: object,
    path: string,
    pathIndex = 0
  ): object {
    ...
    let currentPath =
      pathIndex < pathArray.length ? pathArray[pathIndex] : undefined;
    ...
    if (Array.isArray(json[currentPath])) {
      const filtered = json[currentPath].map(element =>
        this.filterObjectRecursively(element, path, pathIndex + 1)
      );
      return { [currentPath]: filtered };
    } else if (typeof json[currentPath] === 'object') {
      const filtered = this.filterObjectRecursively(
        json[currentPath],
        path,
        pathIndex + 1
      );
