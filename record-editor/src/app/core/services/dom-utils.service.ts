...
export class DomUtilsService {
  ...
  fitEditorHeightFullPage() {
    ...
    const toolbarHeight = window
      .getComputedStyle(document.getElementById('editor-toolbar'), null)
      .getPropertyValue('min-height');
    editorContainer.style.height =
      window.innerHeight - parseInt(toolbarHeight, 10) + 'px';
  }
  ...
  openHostRelativeUrlInNewTab(hostRelativeUrl: string) {
    let newTab = window.open(
      `${environment.baseUrl}/${hostRelativeUrl}`,
      '_blank'
    );
    newTab.focus();
  }
