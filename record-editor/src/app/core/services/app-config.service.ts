...
export class AppConfigService {
  ...
  readonly onConfigChange = Observable.from(onDocumentTypeChange).map(
    documentType => this.configsByType[documentType] || this.configsByType.hep
  );
  ...
  private getHepType(documentTypes: Array<string>): string | undefined {
    return documentTypes.find(
      documentType => this.configsByType[documentType] !== undefined
    );
  }
}
