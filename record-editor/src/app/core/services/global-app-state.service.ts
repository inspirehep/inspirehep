...
export class GlobalAppStateService {
  ...
  readonly hasAnyValidationProblem$ = this.validationProblems$.map(problems =>
    this.hasAnyValidationProblem(problems)
  );

  private hasAnyValidationProblem(problems: SchemaValidationProblems): boolean {
    return Object.keys(problems).some(path => problems[path].length > 0);
  }
}
