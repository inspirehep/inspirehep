...
export default function generateRecordFetchAction({
  ...
  const fetching = (recordId: $TSFixMe) => ({
    type: fetchingActionActionType,
    payload: { recordId }
  });

  const fetchSuccess = (result: $TSFixMe) => ({
    type: fecthSuccessActionType,
    payload: result
  });

  const fetchError = (error: $TSFixMe) => ({
    type: fetchErrorActionType,
    payload: error,
    meta: { redirectableError: true }
  });

  return (recordId: $TSFixMe) => async (dispatch: $TSFixMe, getState: $TSFixMe, http: $TSFixMe) => {
