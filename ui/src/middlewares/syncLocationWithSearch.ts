...
function isSearchPage(location: $TSFixMe) {
...
function removeUiParam(param: $TSFixMe, value: $TSFixMe) {
  return param.startsWith('ui-') ? undefined : value;
}

function isLocationSyncedWithSearchQuery(namespace: $TSFixMe, state: $TSFixMe) {
  const {
    search,
    router: { location },
  } = state;
