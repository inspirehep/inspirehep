...
export function getStorageKeyForReducer(reducerName: $TSFixMe, statePath: $TSFixMe) {
  let pathString = '';
  if (statePath != null) {
...
export function createPersistToStorageMiddleware() {
  const writeStateToStorage = async (state: $TSFixMe) => {
  ...
  return ({
    getState
  }: $TSFixMe) => (next: $TSFixMe) => (action: $TSFixMe) => {
