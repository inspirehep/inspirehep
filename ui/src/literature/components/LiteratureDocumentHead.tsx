...
function LiteratureDocumentHead({ metadata, created }: Props) {
  ...
  return (
    ...
    >
      ...
      {citationPdfUrls &&
        citationPdfUrls.map((link: $TSFixMe) => <meta key={link} name="citation_pdf_url" content={link} />)}

      {authors &&
        authors.map((author: $TSFixMe) => <meta
