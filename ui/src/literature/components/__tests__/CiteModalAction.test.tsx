...
describe('CiteModalAction', () => {
  ...
  beforeAll(() => {
    (citeArticle as $TSFixMe).mockImplementation((format: $TSFixMe, recordId: $TSFixMe) => `Cite ${recordId} in ${format}`);
  });
  ...
  it('calls onCiteFormatChange on format change', () => {
    ...
    (wrapper as $TSFixMe).find(SelectBox).prop('onChange')(CITE_FORMAT_VALUES[1]);
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
  ...
  it('sets modalVisible true on cite click and calls setCiteContentFor with initialCiteFormat if first time', () => {
    ...
    (wrapper.instance() as $TSFixMe).setCiteContentFor = setCiteContentFor;
    wrapper.update();
  ...
  it('sets citeContent for selected format setCiteContentFor', async () => {
    ...
    await (setCiteContentFor as $TSFixMe)('application/vnd+inspire.latex.us+x-latex');
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
  ...
  it('sets modalVisible false onModalCancel', () => {
    ...
    (wrapper.instance().state as $TSFixMe).modalVisible = true;
    wrapper.update();
