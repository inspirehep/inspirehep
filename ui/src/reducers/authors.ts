...
const authorsReducer = (state = initialState, action: $TSFixMe) => {
  switch (action.type) {
    case CLEAR_STATE:
      return initialState;
    case AUTHOR_REQUEST:
    ...
    ? (currentSelection as $TSFixMe).union(selectionUpdate)
    : (currentSelection as $TSFixMe).subtract(selectionUpdate);
    ...
    ? (currentClaimedSelection as $TSFixMe).union(claimedSelectionUpdate)
    : (currentClaimedSelection as $TSFixMe).subtract(claimedSelectionUpdate);
    ...
    ? (currentUnclaimedSelection as $TSFixMe).union(unclaimedSelectionUpdate)
    : (currentUnclaimedSelection as $TSFixMe).subtract(unclaimedSelectionUpdate);
    ...
    ? (currentcanNotClaimPapersSelection as $TSFixMe).union(canNotClaimPapersSelectionUpdate)
    : (currentcanNotClaimPapersSelection as $TSFixMe).subtract(canNotClaimPapersSelectionUpdate);
      return state.set(
        'publicationSelectionCanNotClaim',
