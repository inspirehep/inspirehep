...
describe('CheckboxAggregation', () => {
  ...
  it('render more than 10 after show more clicked', () => {
    ...
    (wrapper.instance() as $TSFixMe).onShowMoreClick();
    wrapper.update();
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
    expect(wrapper).toMatchSnapshot();
  });
  ...
  it('derives selectionMap state from prop selections', () => {
    ...
    const wrapper = shallow(
      <CheckboxAggregation
        // @ts-expect-error ts-migrate(2322) FIXME: Type 'any' is not assignable to type 'never'.
        onChange={jest.fn()}
        // @ts-expect-error ts-migrate(2322) FIXME: Type 'List<any>' is not assignable to type 'never'... Remove this comment to see the full error message
        buckets={List()}
        // @ts-expect-error ts-migrate(2322) FIXME: Type 'string' is not assignable to type 'never'.
        name="Test"
        // @ts-expect-error ts-migrate(2322) FIXME: Type 'string[]' is not assignable to type 'never'.
  ...
  describe('onSelectionChange', () => {
    ...
    it('calls onChange with all selections', () => {
      ...
      (wrapper.instance() as $TSFixMe).onSelectionChange('selected1', true);
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
      expect(onChange).toHaveBeenCalledWith(['selected1']);
      (wrapper.instance() as $TSFixMe).onSelectionChange('selected2', true);
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
