...
function SearchBox({ value, placeholder, onSearch, namespace, className }: Props) {
    ...
    event => {
      ...
      if (newValue && HISTORY_BY_NAMESPACE[namespace]) {
        const options = namespaceHistory
          .filter((searchQuery: $TSFixMe) => searchQuery.includes(newValue))
          .map((searchQuery: $TSFixMe) => ({
          value: searchQuery,
