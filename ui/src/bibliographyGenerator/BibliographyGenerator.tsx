...
function Cite({ children }: CiteProps) {
  return <code>\cite{`{${children}}`}</code>;
}

type BibliographyGeneratorProps = {
    onSubmit: $TSFixMeFunction;
    loading?: boolean;
    data?: $TSFixMe; // TODO: PropTypes.instanceOf(Map)
    citationErrors?: $TSFixMe; // TODO: PropTypes.instanceOf(List)
    error?: $TSFixMe; // TODO: PropTypes.instanceOf(Map)
};

function BibliographyGenerator({ onSubmit, loading, data, citationErrors, error, }: BibliographyGeneratorProps) {
  ...
  return <>
    ...
    <Row type="flex" justify="center" className="overflow-x-auto">
      <Col className="mt3 mb3" xs={24} md={21} lg={16} xl={15} xxl={14}>
        ...
        <Form name="bibliography-generator-form" onFinish={onSubmit} initialValues={{
        format: BIBLIOGRAPHY_GENERATOR_FORMATS[0].value,
    }}>
          <Row>
            <Col>
              <CollapsableForm>
                <(CollapsableForm as $TSFixMe).Section header="Example" key="example">
                  <p>
                    <strong>
...
%8745 citations counted in INSPIRE as of 24 Jun 2020
              ...
              <Row className="pa3 bg-white">
                <Col>
                  ...
                  <Row>
                    <Col span={24}>
                      <Form.Item name="fileupload" rules={[
        {
            required: true,
            message: 'Please select a file',
        },
    ]}>
                        <Dragger {...uploadProps} accept=".tex" name="file" fileList={fileList}>
                  ...
                  {citationErrors && (<Row className="mb3">
                      <Col span={24}>
                        {citationErrors.map((e: $TSFixMe) => <div className="mb2">
                          <Alert type="warning" message={e.get('message')}/>
                        </div>)}
                      </Col>
                    </Row>)}
                  ...
                  <Row type="flex" justify="end">
                    <Form.Item className="no-margin-bottom">
                      <Button type="primary" htmlType="submit" disabled={!fileList} loading={loading}>
                        Submit
                      </Button>
                    </Form.Item>
                  </Row>
