...
describe('OR', () => {
  const divisibleByThreeSchema = mixed().test({
    test: (value: $TSFixMe) => value % 3 === 0,
  });
  const divisibleByTwoSchema = mixed().test({
    test: (value: $TSFixMe) => value % 2 === 0,
  });
  ...
  it('validates if one of the schemas are valid', async (done: $TSFixMe) => {
  ...
  // @ts-expect-error ts-migrate(2582) FIXME: Cannot find name 'it'. Do you need to install type... Remove this comment to see the full error message
  it('invalidates if none of the schemas is valid', async (done: $TSFixMe) => {
