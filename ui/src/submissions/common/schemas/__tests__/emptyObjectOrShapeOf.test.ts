...
describe('emptyObjectOrShapeOf', () => {
  ...
  it('validates empty object', async (done: $TSFixMe) => {
    let validationError;
    try {
  ...
  // @ts-expect-error ts-migrate(2582) FIXME: Cannot find name 'it'. Do you need to install type... Remove this comment to see the full error message
  it('validates empty object if object has only empty properties', async (done: $TSFixMe) => {
    let validationError;
    try {
  ...
  // @ts-expect-error ts-migrate(2582) FIXME: Cannot find name 'it'. Do you need to install type... Remove this comment to see the full error message
  it('validates according to shape schema', async (done: $TSFixMe) => {
    const data = {
      foo: 'bar',
    };
    let validationError;
  ...
  // @ts-expect-error ts-migrate(2582) FIXME: Cannot find name 'it'. Do you need to install type... Remove this comment to see the full error message
  it('invalidates according to shape schema', async (done: $TSFixMe) => {
    const data = {
      notFoo: 'bar',
    };
    let validationError;
