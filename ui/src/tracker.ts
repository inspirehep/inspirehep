...
export function injectTrackerToHistory(history: $TSFixMe) {
...
export async function trackEvent(...args: $TSFixMe[]) {
  if (isTrackerConfigured()) {
    Piwik.push(['trackEvent', ...args]);
  }
}

function getUserEventCategoryFromRoles(userRoles: $TSFixMe) {
...
export async function setUserCategoryFromRoles(userRoles: $TSFixMe) {
  if (isTrackerConfigured()) {
...
export function checkIsTrackerBlocked() {
  return Array.isArray((window as $TSFixMe)._paq) || navigator.doNotTrack === '1'; // eslint-disable-line no-underscore-dangle
