...
export function getCurrentAffiliationsFromPositions(positions: $TSFixMe) {
  return positions.filter((position: $TSFixMe) => position.get('current'));
}

export function getAuthorDisplayName(name: $TSFixMe) {
...
export function getAuthorMetaDescription(author: $TSFixMe) {
  ...
  const affiliationsText = getCurrentAffiliationsFromPositions(
    ...
    .map((position: $TSFixMe) => position.get('institution'))
    .filter(Boolean)
    .join(ITEM_SEPARATOR);
  ...
  const experimentsText = author
    ...
    .map((experiment: $TSFixMe) => experiment.get('name'))
    .filter(Boolean)
    .join(ITEM_SEPARATOR);
