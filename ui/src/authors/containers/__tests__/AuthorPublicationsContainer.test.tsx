...
describe('AuthorPublicationsContainer', () => {
  ...
  it('set assignDifferentProfileView when user has a profile', () => {
    (global as $TSFixMe).CONFIG = { ASSIGN_DIFFERENT_PROFILE_UI_FEATURE_FLAG: true };
    const store = getStoreWithState({
      user: fromJS({
        data: { recid: 3 },
      }),
  ...
  it('set assignViewNoProfile when user logged_in', () => {
    (global as $TSFixMe).CONFIG = { ASSIGN_NO_PROFILE_UI_FEATURE_FLAG: true };
    const store = getStoreWithState({
      authors: fromJS({
        ...initialState,
        publicationSelection: {},
  ...
  // @ts-expect-error ts-migrate(2582) FIXME: Cannot find name 'it'. Do you need to install type... Remove this comment to see the full error message
  it('set assignViewNoProfile when user logged_in', () => {
    (global as $TSFixMe).CONFIG = { ASSIGN_NOT_LOGGED_IN_FEATURE_FLAG: true };
    const store = getStoreWithState({
      authors: fromJS({
        ...initialState,
        publicationSelection: {},
