...
run: start-all sleep setup setup-backoffice setup-inspirehep
...
run-inspirehep: start-inspirehep sleep setup-inspirehep

run-backoffice: start-backoffice sleep setup-backoffice

start-inspirehep:
	echo -e "\033[0;32m Starting HEP. \033[0m"
	docker compose up -d es cache db mq s3 hep-worker hep-web record-editor hep-ui ui
	echo -e "\033[0;32m HEP Started. \033[0m"

start-backoffice:
	echo -e "\033[0;32m Starting Backoffice. \033[0m"
	docker compose up -d es cache db mq airflow-init airflow-worker airflow-webserver airflow-triggerer airflow-scheduler backoffice-webserver
