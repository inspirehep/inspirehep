...
def _success_criteria(record):
    ...
    print("this is the success criteria")
    print(record)
    return record
...
def author_create_initialization_sensor_dag8():
    """
    ...
    def wow(**context):
        print("wow")
    waiting_for_author = SqlSensor(
        task_id="waiting_for_partner",
        conn_id="inspire_db_connection",
        sql="SELECT * FROM workflows_workflow where workflow_type = 'AUTHOR_CREATE' and status = 'queued'",
...
author_create_initialization_sensor_dag8()
