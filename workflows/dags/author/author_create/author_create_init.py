...
def author_create_initialization_dag():
    ...
    def author_check_approval_branch(**context: dict) -> None:
        ...
        workflow_data = context["params"]["workflow"]["data"]
        if "decisions" not in workflow_data or len(workflow_data["decisions"]) == 0:
            return "set_author_create_workflow_status_to_approval"
        elif workflow_data["decisions"][0]["value"] in ["accept_curate", "accept"]:
            return "trigger_approve"
    ...
    sub_task = set_submission_number(data_with_schema)

    trigger_approve = TriggerDagRunOperator(
        task_id="trigger_approve",
