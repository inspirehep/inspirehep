...
class TestSetWorkflowStatus:
    ...
    def test_set_workflow_status_to_error(self):
        self.workflow_management_hook.set_workflow_status(
            status_name="running", workflow_id=self.context["run_id"]
        )
        set_workflow_status_to_error(context=self.context, collection=AUTHORS)
        assert (
            self.workflow_management_hook.get_workflow(self.context["run_id"])["status"]
            == "error"
        )
