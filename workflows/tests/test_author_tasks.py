...
class TestAuthorCreateInit:
    ...
    def test_create_author_create_curation_ticket(
        self, mock_create_ticket, mock_create_ticket_entry
    ):
        self.context["ti"].xcom_pull.return_value = 123456
    ...
    def test_author_check_approval_branch(self, mock_get_workflow):
        task = self.dag.get_task("author_check_approval_branch")
        mock_get_workflow.return_value = {
            "data": {},
            "decisions": [{"action": "accept_curate"}],
        }
        result = task.python_callable(
            params={
                "workflow_id": 1,
                "workflow": {"data": {}},
                "data": {"value": "accept_curate"},
            }
        )
